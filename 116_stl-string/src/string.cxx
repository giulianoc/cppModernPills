...
int main() {
  ...
  s1.capacity(); // size of storage space currently allocated to s1 (it must be
                 // bigger than size)
  s1.reserve(100); // allocate 100 chars for the string (change the capacity)
  s1.reserve(5); // since the string is "Goodbye", size will be 7, even capacity
                 // will be 7
  ...
  // and like vector, string doesn't have push_front() and pop_front() because
  // are not efficient
  ...
  s1.assign(
      s2, 2,
      4); // agon when the first parameter is a string, 2 is start and 4 is size
  s1.assign("Wisdom");
  s1.assign("Wisdom",
            3);      // Wis  when the first parameter is a cstring, 3 is size
  s1.assign(3, 'x'); // xxx
  ...
  s1.replace(2, 5, s2, 3,
             3); // abgon def   2: start, 5: size, 3: start, 3: size

  // remove if a digit
  {
    string test1 = "N/A";
    test1.erase(remove_if(test1.begin(), test1.end(), ::isdigit), test1.end());
    cout << "test remove if a digit: " << test1 << endl;

    // remove if not a digit
    string test2 = "12345Kib";
    test2.erase(remove_if(test2.begin(), test2.end(),
                          [](unsigned char c) { return !isdigit(c); }),
                test2.end());
    cout << "test remove if not a digit: " << test2 << endl;
  }
  ...
  cout << "Replace." << endl;
  string test =
      "{	\"Type\": \"Process\",	\"Label\": \"1\",	\"Variables\": "
      "{  		\"Title\": \"My Title of the content\"	},	"
      "\"Task\": {		\"Label\": \"nome univoco del task all'interno "
      "del processo\",		\"Type\": \"ContentIngestion\",		"
      "\"Parameters\": {		\"SourceURL\": "
      "\"http://techslides.com/demos/sample-videos/small.mp4\",		"
      "	\"SourceFileName\": \"123456.mp4\",			"
      "\"ContentType\": \"video\",			\"userData\": { "
      "\"MyData\": \"this is my data\" },			\"Title\": "
      "\"_Title_\"		}	}}";
  ...
  size_t found = s1.find("doing"); // look for the entire string "doing", found:
                                   // 17, string::npos in case it is not found
  found = s1.find("doing", 20);    // found: 35, 20: start
  found = s1.find("doing well", 0); // found: 35
  found = s1.find("doing well", 0,
                  5); // found: 17, 0: start, 5: looking 5 chars ("doing")
  found = s1.find_first_of("doing"); // found: 6, looking for one char (in our
                                     // case '0' is found first)
  found = s1.find_first_of("doing", 10); // found: 12, 10: start
  found = s1.find_first_of(
      "doing", 10, 1); // found: 17, 10: start, only the first chat ('d')
  found = s1.find_first_not_of(
      "doing"); // found: 17, 10: start, only the first chat ('d')

  found = s1.find_last_of(
      "doing"); // found: 0 (Searches the string for the last character that
  ...
  // next is end with (StringUtils::endWith)
  string suffix(".mp4");
  if (s1.size() >= suffix.size() &&
      0 == s1.compare(s1.size() - suffix.size(), suffix.size(), suffix))
    ;

  // next is start with (StringUtils::startWith)
  ...
  s1 = "190 monkeys";
  size_t pos;
  i = stoi(s1, &pos); // i = 190, pos will be the index of char where stoi has
                      // stopped to parsing the number, pos: 3
  ...
  string::iterator itr = search_n(s1.begin(), s1.begin() + 20, 2,
                                  's'); // itr --> the first 's', search 2
                                        // consecutive 's' in the first 20 chars
  s1.erase(itr, itr + 5);               // "ss is" would be removed
  s1.insert(itr, 3, 's');
  s1.replace(
      itr, itr + 3, 3,
      'y'); // replace 3 chars starting from itr with 3 'y', replace substring

  // replace the first substring with another substring. The initial buffer is
  // modified
  {
    cout << endl;
    ...
    if (strbase.find(strToBeReplaced) != string::npos)
      strbase.replace(strbase.find(strToBeReplaced), strToBeReplaced.length(),
                      strToReplace);

    cout << "new strbase: " << strbase << endl;
  }
  replace(s1.begin(), s1.end(), 'e',
          ' '); // replace 'e' with ' ', it replace characters
  ...
  s1 = "abcdefg";
  rotate(s1.begin(), s1.begin() + 3,
         s1.end()); // "defgabc" s1.begin() + 3 becomes the first char
  ...
  {
    // given a string, convert it to lowercase and count the length of the
    // non-empty words of the string
    s1 = "Hello       World";

    cout << "Lowercase on a different string. s1: " << s1 << ", s2: " << s2
         << endl;
    s2.resize(s1.size());
    ...
    cout << "Splitting in words (option 1): "
         << s1; // drawback. it can’t split on anything else than spaces, which
                // can be an issue, like for parsing a CSV
